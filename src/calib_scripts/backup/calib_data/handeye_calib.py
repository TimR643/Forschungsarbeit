...
def rvec_tvec_to_matrix(rvec, tvec):
    ...
    T = np.eye(4)
    T[:3, :3] = R
    T[:3, 3] = np.array(tvec).flatten()   # ✅ ensure shape (3,)
    return T
...
def compute_handeye(samples):
    ...
    for i in range(len(samples) - 1):
        ...
        A = np.linalg.inv(T_robot_prev) @ T_robot_curr      # Robot motion
        B = np.linalg.inv(T_cam_prev) @ T_cam_curr          # ✅ Camera motion (fixed)
    ...
    T = np.eye(4)
    T[:3, :3] = R_cam2ee
    T[:3, 3] = t_cam2ee.flatten()   # ✅ ensure shape (3,)
    return T

if __name__ == "__main__":
    samples = load_samples(".")    # ✅ Load from current folder
    if len(samples) < 2:
