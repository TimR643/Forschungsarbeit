...
if __name__ == "__main__":
    ...
    print("\n🎯 Raw Camera-to-End-Effector Transform:")
    print(np.round(T_cam2ee, 4))

    # ===========================================
    # 🔧 MANUAL CORRECTIONS – adjust as needed
    # ===========================================

    # 1. Flip Z (camera facing wrong direction)
    # T_cam2ee[:3, 3] *= [1, 1, -1]

    # 2. Apply 90° rotation about X (optical -> URDF)
    R_fix = cv2.Rodrigues(np.array([np.pi / 2, 0, 0]))[0]
    T_cam2ee[:3, :3] = T_cam2ee[:3, :3] @ R_fix

    # 3. Tiny offset to push marker forward (optional tuning)
    T_cam2ee[:3, 3] += np.array([0.01, 0.00, 0.00])  # Δx, Δy, Δz in meters

    # ===========================================

    print("\n✅ Corrected Camera-to-End-Effector Transform:")
    print(np.round(T_cam2ee, 4))
